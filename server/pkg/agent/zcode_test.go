...
//     upstream error string "model not available"; when "session_not_found",
//     it returns the bridge's dead-session error.
...
while IFS= read -r line; do
  ...
  case "$line" in
    ...
    *'"method":"session/new"'*)
      ...
      printf '{"jsonrpc":"2.0","id":%s,"result":{"sessionId":"ses_zc_new","configOptions":[{"id":"model","currentValue":"glm-5.3","options":[{"value":"glm-5.3","name":"GLM-5.3"}]},{"id":"thought","category":"thought_level","currentValue":"max","options":[{"value":"low","name":"low"},{"value":"high","name":"high"},{"value":"max","name":"max"}]}]}}\n' "$id"
      ;;
      ...
      ;;
	    *'"method":"session/resume"'*)
	      if [ "$ZC_RESUME_ERROR" = "session_not_found" ]; then
	        ...
	        exit 0
	      elif [ "$ZC_RESUME_ERROR" = "transient" ]; then
	        printf '{"jsonrpc":"2.0","id":%s,"error":{"code":-32000,"message":"temporary backend unavailable"}}\n' "$id"
	        exit 0
      fi
      if [ "$ZC_REPLAY" = "1" ]; then
...
func TestZcodeBackendRejectsUnsupportedAgentMcpConfig(t *testing.T) {
	t.Parallel()

	dir := t.TempDir()
	fakePath := filepath.Join(dir, "zcode-acp-server")
	writeTestExecutable(t, fakePath, []byte(fakeZcodeACPScript()))
	backend, err := New("zcode", Config{ExecutablePath: fakePath, Logger: slog.Default()})
	if err != nil {
		t.Fatalf("new zcode backend: %v", err)
	}
	_, err = backend.Execute(context.Background(), "hi", ExecOptions{
		McpConfig: json.RawMessage(`{"mcpServers":{"local":{"command":"example-mcp"}}}`),
	})
	if err == nil || !strings.Contains(err.Error(), "not supported by the pinned zcode-acp-server revision") {
		t.Fatalf("expected visible unsupported MCP error, got %v", err)
	}
}
...
// session dying before the model switch: the id must be cleared so the daemon
// retries fresh instead of looping on a dead pointer.
func TestZcodeBackendSetModelSessionNotFoundClearsSessionID(t *testing.T) {
	t.Parallel()

	result, _ := runZcodeTest(t, map[string]string{"ZC_SET_MODEL_ERROR": "session_not_found"}, ExecOptions{
...
func TestZcodeBackendSendsGracefulCancel(t *testing.T) {
	...
	case result := <-session.Result:
		if result.Status != "aborted" {
			t.Fatalf("expected aborted after cancel, got %q (error=%q)", result.Status, result.Error)
		}
...
// TestZcodeBackendCancelBackstopReapsUncooperativeBridge proves WaitDelay
// closes the ACP pipes and unblocks the task even when the bridge ignores both
// session/cancel and stdin EOF.
func TestZcodeBackendCancelBackstopReapsUncooperativeBridge(t *testing.T) {
	origDelay := zcodeCancelWaitDelay
	zcodeCancelWaitDelay = 100 * time.Millisecond
	t.Cleanup(func() { zcodeCancelWaitDelay = origDelay })
	...
	case result := <-session.Result:
		if result.Status != "aborted" {
			t.Fatalf("expected aborted after forced reap, got %q (error=%q)", result.Status, result.Error)
		}
	case <-time.After(3 * time.Second):
		t.Fatal("uncooperative bridge was not reaped by cancellation backstop")
	}
}
...
func TestZcodeBackendTransientResumeFailurePreservesSessionID(t *testing.T) {
	t.Parallel()

	result, _ := runZcodeTest(t, map[string]string{"ZC_RESUME_ERROR": "transient"}, ExecOptions{
		ResumeSessionID: "ses_zc_resumed",
	})
	if result.Status != "failed" {
		t.Fatalf("expected status=failed, got %q (error=%q)", result.Status, result.Error)
	...
	if result.SessionID != "ses_zc_resumed" {
		t.Fatalf("transient resume error cleared session id: got %q", result.SessionID)
	}
}
...
// TestZcodeBackendUsesRuntimeDefaultThinking pins the company-fork contract:
// until its shared ACP effort support lands, a stale persisted level cannot
// inject an unverified token into ZCode. The task still completes using the
// runtime's own default.
func TestZcodeBackendUsesRuntimeDefaultThinking(t *testing.T) {
	t.Parallel()
	...
	if strings.Contains(requests, `"method":"session/set_config_option"`) {
		t.Errorf("unexpected session/set_config_option request; company fork must use the runtime default:\n%s", requests)
	}
}
