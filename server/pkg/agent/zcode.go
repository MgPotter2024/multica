...
//   - This Multica release does not expose the bridge's model-specific thought
//     vocabulary yet. A requested thinking override is ignored with a warning,
//     and ZCode keeps its configured runtime default.
...
func (b *zcodeBackend) Execute(ctx context.Context, prompt string, opts ExecOptions) (*Session, error) {
	...
	// Parse mcp_config so malformed input still fails visibly. The pinned
	// production bridge revision predates zcode-acp#43 and does not forward
	// client-provided servers into the official runtime, so reject a non-empty
	// list instead of pretending the configuration took effect.
	mcpServers, err := buildACPMcpServers(opts.McpConfig, b.cfg.Logger)
	if err != nil {
		return nil, fmt.Errorf("zcode: invalid mcp_config: %w", err)
	}
	if len(mcpServers) > 0 {
		return nil, fmt.Errorf("zcode: agent mcp_config is not supported by the pinned zcode-acp-server revision; configure MCP in ZCode itself")
	}
	...
	go func() {
		...
		bridgeVersion := zcodeACPAgentVersion(initResult)
		...
		mcpServers = filterACPMcpServersByCapability(mcpServers, extractACPMcpCapabilities(initResult), "zcode", b.cfg.Logger)
		...
		if opts.ResumeSessionID != "" {
			...
			if err != nil {
				...
				// found". Return an empty session id so the daemon's
				// resume-failure fallback retries
		...
		if opts.ThinkingLevel != "" {
			b.cfg.Logger.Warn("zcode thinking override is not supported by this Multica release; using the runtime default",
...
func zcodeACPAgentVersion(raw json.RawMessage) string {
	var result struct {
		AgentInfo struct {
	...
	return strings.TrimSpace(result.AgentInfo.Version)
}
...
func zcodeSendSessionCancel(c *hermesClient) {
	...
	_ = c.sendNotification("session/cancel", map[string]any{"sessionId": sid})
}
