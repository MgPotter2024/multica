...
Z.ai's official desktop coding agent. Multica launches its bundled runtime through the Apache-2.0 community bridge `zcode-acp-server`, which translates ZCode's local app-server protocol to ACP. Sessions resume and models are discovered dynamically. Configure MCP inside ZCode; the pinned bridge does not yet forward Multica-managed MCP.

| | |
|---|---|
| Daemon looks for | `zcode-acp-server`, plus either the official `zcode` command or `ZCode.app` in a standard macOS Applications directory |
| Install | Install the official [ZCode desktop app](https://zcode.z.ai/en/docs/install), then build or install [`zcode-acp-server`](https://github.com/william0wang/zcode-acp) from a revision containing the runtime-preferences handshake fix in `william0wang/zcode-acp#38`. The published npm package `0.1.0` is older and is not compatible with current ZCode releases. |
| Authentication | Sign in once in ZCode, or configure ZCode's API-key mode. The bridge reuses ZCode's local configuration; do not put the provider key in the Multica agent environment. |
| Notes | Tested with ZCode 3.7.7 and bridge commit `a5cb8a1`. Override the bridge path with `MULTICA_ZCODE_PATH` and the default model with `MULTICA_ZCODE_MODEL`; use the exact discovered id, such as `multica-bigmodel\glm-5.2`. |
