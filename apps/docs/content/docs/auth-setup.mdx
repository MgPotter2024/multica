...
**Yes.** A live pending workspace invitation is an implicit per-email signup allowance. Existing users can sign in as usual, and a new user can request a verification code and create an account with the email address on a pending, unexpired invitation.

Uninvited users and users whose invitations are expired, accepted, declined, or revoked remain blocked when `ALLOW_SIGNUP=false`.
