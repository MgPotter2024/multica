...
在 Multica 里，一个**运行时**（runtime）就是你机器上的守护进程，配上守护进程在 `PATH` 里扫到的某一款 AI 编程工具。如果上手引导的 "连接运行时" 这一步显示 **未检测到支持的工具**，说明守护进程扫了 `PATH`，但 16 款它认得的工具一个都没找到。装下面任意一款（或几款），回到这一步重新扫描，几秒内运行时就会出现。
...
xAI 的 Grok Build CLI（`grok`）。Multica 通过 ACP 驱动它；会话恢复、动态模型发现、MCP 和 `.grok/skills/` 原生 skill 目录均可用。

| | |
|---|---|
| 守护进程扫描 | `grok` |
| 安装 | 安装 Grok Build CLI，并确保 `grok` 在 `PATH` 上。|
| 认证 | 跑 `grok login` 完成交互式登录。|
| 备注 | 可用 `MULTICA_GROK_PATH` 覆盖路径、`MULTICA_GROK_MODEL` 设置默认模型；最低 CLI 版本为 `0.2.89`。|

### ZCode（Z.ai）

Z.ai 官方桌面编程智能体。Multica 通过 Apache-2.0 社区适配层 `zcode-acp-server` 启动官方 ZCode 自带的运行时，并把其本地 app-server 协议转换成 ACP。会话恢复和动态模型发现可用。MCP 请在 ZCode 内配置；锁定版本的适配层暂不转发 Multica 管理的 MCP。

| | |
|---|---|
| 守护进程扫描 | `zcode-acp-server`，以及官方 `zcode` 命令或 macOS 标准位置中的 `ZCode.app`；两层都要存在 |
| 安装 | 先按[官方说明](https://zcode.z.ai/en/docs/install)安装 ZCode 桌面端，再从 [`zcode-acp-server`](https://github.com/william0wang/zcode-acp) 安装包含 `william0wang/zcode-acp#38` 握手修复的版本。npm 上的 `0.1.0` 较旧，不兼容当前 ZCode。|
| 认证 | 在 ZCode 内登录一次，或配置 ZCode 的 API key 模式。适配层复用 ZCode 本机配置，不要把供应商 key 放进 Multica 智能体环境变量。|
| 备注 | 已验证 ZCode 3.7.7 与适配层提交 `a5cb8a1`。可用 `MULTICA_ZCODE_PATH` 覆盖适配层路径，用 `MULTICA_ZCODE_MODEL` 设置默认模型；请填写发现结果中的精确 id，例如 `multica-bigmodel\glm-5.2`。|
...
4. **回到上手引导。** "连接运行时" 这一步会一直轮询，几秒内就能扫到新运行时，不需要手动刷新。
