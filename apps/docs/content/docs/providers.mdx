...
| **ZCode** | Z.ai | ✅ (ACP `session/resume`) | ❌ | `.agents/skills/` | Dynamic discovery (ACP config options) |
...
From Z.ai. Multica drives the official ZCode runtime through the Apache-2.0 community bridge [`zcode-acp-server`](https://github.com/william0wang/zcode-acp), which translates ZCode's local app-server protocol to ACP. The bridge launches the runtime bundled with the official ZCode desktop app and reuses ZCode's local API-key or account configuration. **Session resumption works** through ACP `session/resume`; models are discovered from the bridge's ACP config options and selected per task. Multica-managed MCP is unavailable in the pinned bridge revision: configure MCP inside ZCode itself. Skills use ZCode's native project path, `.agents/skills/`. Multica requires a bridge revision containing the runtime-preferences handshake fix from `william0wang/zcode-acp#38`; the published npm package `0.1.0` predates that fix.
...
**Of the 16 tools, twelve consume `mcp_config`: Claude Code, CodeBuddy, Codex, Cursor, Grok, Hermes, Kimi, Kiro CLI, OpenCode, OpenClaw, Qoder, and Trae**. Three tools (Antigravity, Copilot, and Pi) ignore it. ZCode rejects a non-empty value because the pinned bridge revision cannot forward it; configure MCP in ZCode itself.
