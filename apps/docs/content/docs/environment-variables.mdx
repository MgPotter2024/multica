...
| `MULTICA_ZCODE_PATH` | `zcode-acp-server` | Path to the ZCode ACP bridge executable |
| `MULTICA_ZCODE_MODEL` | empty | Exact model id advertised by the ZCode bridge (for a custom provider, for example `multica-bigmodel\glm-5.2`) |
| `ZCODE_BIN` | `zcode` or the detected ZCode.app entry point | Official ZCode CLI path passed to the ACP bridge |
| `ZCODE_NODE` | bridge auto-discovery | Optional Node.js executable used to launch ZCode's bundled `.cjs` entry point |
