...
**可能です。** 有効な保留中のワークスペース招待は、そのメールアドレスに対する暗黙のサインアップ許可になります。既存ユーザーは通常どおりログインでき、新規ユーザーは保留中かつ有効期限内の招待に記載されたメールアドレスで認証コードを要求してアカウントを作成できます。

`ALLOW_SIGNUP=false` の場合、招待されていないユーザー、および招待が期限切れ、受諾済み、辞退済み、または取り消し済みのユーザーは引き続きブロックされます。
