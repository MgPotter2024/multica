...
**가능합니다.** 유효한 대기 중 워크스페이스 초대는 해당 이메일에 대한 암시적 가입 허용으로 처리됩니다. 기존 사용자는 평소처럼 로그인할 수 있고, 새 사용자는 대기 중이며 만료되지 않은 초대의 이메일로 인증 코드를 요청하고 계정을 만들 수 있습니다.

`ALLOW_SIGNUP=false`인 경우 초대받지 않은 사용자와 만료, 수락, 거절 또는 취소된 초대의 사용자는 계속 차단됩니다.
