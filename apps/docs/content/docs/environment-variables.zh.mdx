...
| `MULTICA_ZCODE_PATH` | `zcode-acp-server` | ZCode ACP 适配层的可执行文件路径 |
| `MULTICA_ZCODE_MODEL` | 空 | ZCode 适配层广播的精确模型 id（自定义 provider 例如 `multica-bigmodel\glm-5.2`）|
| `ZCODE_BIN` | `zcode` 或自动发现的 ZCode.app 入口 | 传给 ACP 适配层的官方 ZCode CLI 路径 |
| `ZCODE_NODE` | 由适配层自动发现 | 可选的 Node.js 可执行文件，用来启动 ZCode 自带的 `.cjs` 入口 |
