...
The invited email **does not need to be registered with Multica in advance** — if no account exists, a live pending invitation lets that email create one without a separate allowlist entry. See [Auth setup](/auth-setup#can-you-still-invite-people-when-signup-is-disabled).
