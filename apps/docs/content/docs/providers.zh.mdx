...
| **ZCode** | Z.ai | ✅（ACP `session/resume`）| ❌ | `.agents/skills/` | 动态发现（ACP 配置项）|
...
xAI 出品。Multica 通过 `grok --no-auto-update agent --always-approve stdio` 以 ACP 驱动 Grok Build CLI。会话恢复通过 ACP `session/load` 工作，模型从 `session/new` 动态发现，MCP 配置通过 ACP `mcpServers` 传入，skill 写入 `.grok/skills/`。

### ZCode

Z.ai 出品。Multica 通过 Apache-2.0 社区适配层 [`zcode-acp-server`](https://github.com/william0wang/zcode-acp) 驱动官方 ZCode 运行时；适配层把 ZCode 的本地 app-server 协议转换成 ACP，并复用 ZCode 本机保存的 API key 或登录配置。会话恢复通过 ACP `session/resume` 工作，模型从 ACP 配置项动态发现并可按任务选择。锁定版本的适配层暂不支持 Multica 管理的 MCP，请在 ZCode 内配置 MCP。Skill 使用 ZCode 原生的项目路径 `.agents/skills/`。Multica 要求适配层包含 `william0wang/zcode-acp#38` 的 runtime-preferences 握手修复；npm 上的 `0.1.0` 早于这个修复，不兼容当前 ZCode。
...
**16 款工具里有 12 款实际消费 `mcp_config`：Claude Code、CodeBuddy、Codex、Cursor、Grok、Hermes、Kimi、Kiro CLI、OpenCode、OpenClaw、Qoder、Trae CLI**。Antigravity、Copilot、Pi 会忽略该字段。ZCode 会拒绝非空配置，因为锁定版本的适配层还不能转发；请改在 ZCode 内配置 MCP。
